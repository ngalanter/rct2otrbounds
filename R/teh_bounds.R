...
#' @examples #tbd
#'
teh_closed_form_bounded <- function(m, M, s2_1, s2_0, n_1=NULL, n_0=NULL,
                                    mean_1=NULL, mean_0=NULL,
                                    conf.int = FALSE, level = 0.95)
{
  ...
  if(conf.int){
    ...
    var_low <- ( max((m-mean_1)^2,(M-mean_1)^2)*s2_1 - s2_1^2 ) * (1/nu_hat - 1)^2 / r_1 +
      ( max((m-mean_0)^2,(M-mean_0)^2)*s2_0 - s2_0^2 ) * (nu_hat - 1)^2 / r_0

    #upper bound on the variance of general upper bound on teh
    var_high <- ( max((m-mean_1)^2,(M-mean_1)^2)*s2_1 - s2_1^2 ) * (1/nu_hat + 1)^2 / r_1 +
      ( max((m-mean_0)^2,(M-mean_0)^2)*s2_0 - s2_0^2 ) * (nu_hat + 1)^2 / r_0

    #lower confidence interval limit for the bounds
    ci_low <- stats::qnorm(q_low, mean = point_low, sd = sqrt(var_low/n))

    #upper confidence interval limit for the bounds
    ci_high <- stats::qnorm(q_high, mean = point_high, sd = sqrt(var_high/n))
  } else {
    ci_low = NULL
    ci_high = NULL
  }
  return(list(estimates = c(point_low,point_high),conf.int= c(ci_low,ci_high)))
...
#'
#' @export
treatment_effect_heterogeneity_bound <- function(s2_1,s2_0,
                                                 n_1 = NULL,n_0 = NULL,
                                                 mean_1 = NULL,mean_0 = NULL,
                                                 m = NULL,M = NULL,level = 0.95,
                                                 bounded_outcome = F, conf.int = F)
  {
  ...
  if(bounded_outcome){
    ...
    return(teh_closed_form_bounded(m=m,M=M,s2_1=s2_1,s2_0=s2_0,
                                   n_1=n_1,n_0=n_0,mean_1=mean_1,mean_0=mean_0,
                                   conf.int = conf.int, level = level))
  } else{

    if(conf.int) warning("No confidence interval can be provided for unbounded outcomes")

    return(list(estimates = teh_closed_form_unbounded(s2_1,s2_0),
